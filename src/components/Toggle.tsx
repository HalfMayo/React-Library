...
export default function Toggle({info, isOpen, i, onClick, className, width = "500px", compressed = false, bgTitle, bgText} : ToggleElProps) {

    return(
        <li className="list-none" style={{width: `${width}`}}>
            <div aria-label={isOpen === i ? "Hide" : "Show"} onClick={() => onClick(i)} className={`p-4 text-base m-0 flex items-center justify-between gap-4 w-full ${className} ${bgTitle}`}>
            ...
            <div className={`${bgText} overflow-hidden transition-[max-height] ease-linear ${isOpen === i ? "max-h-[1000px] duration-[1500ms]" : "max-h-0 duration-1000"}`}>
                <div className="p-4"> 
                            <div className={compressed ? "pr-2 max-h-40 overflow-y-auto scrollbar": ""}>
