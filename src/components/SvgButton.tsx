...
export default function SvgButton({classNameButton, classNameIcon, label, svg, type, color = "text-on-surface", width = 24, height = 24, onClick, face = "#FFFBFE", faceshadow = "#EBE6EA", feature = "#1C1B1F", disabled = false, dragControls, onMouseLeave, onMouseOver} : SvgProps) {
    ...
    const strW = width.toString() + "px";
        ...
        return(
            <button className={`w-12 h-12 flex items-center justify-center bg-transparent ${classNameButton} ${disabled ? "text-disabled" : color}`} aria-label={label} onClick={onClick} type={type} onPointerDown={(event) => dragControls?.start(event)} onMouseLeave={onMouseLeave} onMouseOver={onMouseOver}>
                <Icon className={classNameIcon} width={
                    typeof width === "number"
                        ? strW
                        : width
